...
public class Cornwall_Login_from_FUN {
	
	//INSTANTIATE WEBDRIVER
			WebDriver driver_Cornwall_Login_from_FUN;
	...
	@Test(priority=6)
	@Parameters("browser")
	public void i_am_a_registered_user_of_Cornwall_and_I_open_a_browser(String browser) {
		//If browser is firefox
		if(browser.equalsIgnoreCase("firefox")) {
			driver_Cornwall_Login_from_FUN = new FirefoxDriver();	
		}
	
	//If browser is safari	
		else if (browser.equalsIgnoreCase("safari")) { 
			driver_Cornwall_Login_from_FUN= new SafariDriver();
			driver_Cornwall_Login_from_FUN.manage().window().maximize();
		} 
		
	//If browser is chrome	
		else if (browser.equalsIgnoreCase("chrome")) { 
			driver_Cornwall_Login_from_FUN= new ChromeDriver();
		} 
	}

	@Given("I navigate to a premium article on Cornwall")
	@Test(priority=7)
	public void i_navigate_to_a_premium_article_on_Cornwall() throws InterruptedException, IOException {
		//open a premium article on reaction	
		driver_Cornwall_Login_from_FUN.get("https://cornwallreports.co.uk/european-union-puts-1-8-million-into-new-helicopter-link-to-isles-of-scilly/");
						Thread.sleep(10000);
						
					//verify that the premium article is opened	
						String reaction_navigation= driver_Cornwall_Login_from_FUN.getCurrentUrl();
					    try
						{
					    	AssertJUnit.assertTrue(reaction_navigation.contains("https://cornwallreports.co.uk/european-union-puts-1-8-million-into-new-helicopter-link-to-isles-of-scilly/"));
						}catch(AssertionError e0)
						{
							System.out.println("Browser did not open Cornwall ");
							throw e0;
						}
					    System.out.println("Cornwall is ready to be tested");
					
					//scroll for first use notice
					  	JavascriptExecutor js = (JavascriptExecutor)driver_Cornwall_Login_from_FUN;
					  	js.executeScript("window.scrollBy(0,500)");    
					
					//Screenshot 1 of reaction first use notice
						File scrFile = ((TakesScreenshot)driver_Cornwall_Login_from_FUN).getScreenshotAs(OutputType.FILE);
					...
					//Screenshot 2 of reaction first use notice
						File scrFile1 = ((TakesScreenshot)driver_Cornwall_Login_from_FUN).getScreenshotAs(OutputType.FILE);
						String timestamp1 = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().getTime());
						FileUtils.copyFile(scrFile1, new File("/Users/jay/Desktop/cornwall/"+"_"+timestamp1+"_"+"jpg" ));	
	}

	@When("I click on Login from Cornwall First use notice")
	@Test(priority=8)
	public void i_click_on_Login_from_Cornwall_First_use_notice() throws InterruptedException {
		
		
		Thread.sleep(6000);
		
		driver_Cornwall_Login_from_FUN.findElement(By.xpath("/html/body/div[1]/button")).click();
		Thread.sleep(4000);
		
		
		//call first use notice elements of cornwall
		Thread.sleep(15000);
		Cornwall_Reports_First_Use_Notice Cornwall_First_Use_Notice = new Cornwall_Reports_First_Use_Notice(driver_Cornwall_Login_from_FUN);
	...
	@Test(priority=9)
	public void i_verify_that_the_Login_Button_Navigates_me_to_the_login_page_to_log_me_in_to_Cornwall() throws IOException, InterruptedException {
	...
	//get current url of login page
		String actual_click_on_login_from_reaction_FUN= driver_Cornwall_Login_from_FUN.getCurrentUrl();
	...
	@Then("entering the correct email and password logs me in to Cornwall")
	@Test(priority=10)
	public void entering_the_correct_email_and_password_logs_me_in_to_Cornwall() throws IOException, InterruptedException {
		Login_Page_Elements Login_Pop_First_Use = new Login_Page_Elements(driver_Cornwall_Login_from_FUN);
		Thread.sleep(2000);
	...
	@Then("verify that I am being navigated to the same article on Cornwall")
	@Test(priority=11)
	public void verify_that_I_am_being_navigated_to_the_same_article_on_Cornwall() throws IOException, InterruptedException {
		//scroll for first use notice
	  	JavascriptExecutor js2 = (JavascriptExecutor)driver_Cornwall_Login_from_FUN;
	...
	//click ok on authorise charge notice
		Thread.sleep(10000);
		Authorise_charge_notice_popbitch authy_popbitch = new Authorise_charge_notice_popbitch(driver_Cornwall_Login_from_FUN);
		authy_popbitch.authorise_charge_notice_click_continue();
		Thread.sleep(3000);
		
		
		//Click on the green tab and 
				PopbitchFirstUseNoticeElements pop_first_use_top_up_from_wallet2 = new PopbitchFirstUseNoticeElements(driver_Cornwall_Login_from_FUN);
				pop_first_use_top_up_from_wallet2.click_on_green_tab();
						
	//check if the wallket is being deducted by 30 p
		Popbitch_Wallet_Elements_staging wallet_elements_1 = new Popbitch_Wallet_Elements_staging(driver_Cornwall_Login_from_FUN);
	...
	//String screenshot_name_3= "Popbitch_full_article_appears";
		...
		File scrFile_3 = ((TakesScreenshot)driver_Cornwall_Login_from_FUN).getScreenshotAs(OutputType.FILE);		
	...
	//String screenshot_name_4= "Popbitch_full_article_appears_2";
		String timestamp_4 = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().getTime());
						File scrFile_4 = ((TakesScreenshot)driver_Cornwall_Login_from_FUN).getScreenshotAs(OutputType.FILE);		
						FileUtils.copyFile(scrFile_4, new File("/Users/jay/Desktop/cornwall/"+"_"+timestamp_4+"_"+"jpg" ));		
						Thread.sleep(3000);		
			
						JavascriptExecutor js1 = (JavascriptExecutor)driver_Cornwall_Login_from_FUN;
						js1.executeScript("window.scrollBy(0,500)");		
						String timestamp_5 = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(Calendar.getInstance().getTime());
						File scrFile_5 = ((TakesScreenshot)driver_Cornwall_Login_from_FUN).getScreenshotAs(OutputType.FILE);		
						FileUtils.copyFile(scrFile_5, new File("/Users/jay/Desktop/cornwall/"+"_"+timestamp_5+"_"+"jpg" ));
						Thread.sleep(1000);
						
					//Verify the user landed on the same article
						String Login_From_PopBitch_Frist_Use_Notice_Current_Url1= driver_Cornwall_Login_from_FUN.getCurrentUrl();	   
					    String Login_From_PopBitch_Frist_Use_Notice_Popbitch_Login_Url1 = "https://cornwallreports.co.uk/european-union-puts-1-8-million-into-new-helicopter-link-to-isles-of-scilly/";
